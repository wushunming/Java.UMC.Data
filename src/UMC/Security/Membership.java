...
public class Membership extends UMC.Data.DataProvider {
    ...
    public UMC.Security.Principal Authorization(String SessionKey, String contentType) {

        UUID sessionKey = Utility.uuid(SessionKey, true);
    ...
    public List<Identity> Identity(String... names) {
        List<Identity> ids = new LinkedList<>();
        if (names.length > 0) {
            List<String> ns = new LinkedList<>();
            UMC.Data.Database.instance().objectEntity(UMC.Data.Entities.User.class)
                    .where().and().in(new UMC.Data.Entities.User().Username(names[0]), names)
                    .entities().query(dr ->
            {
                ns.removeIf(n -> dr.Username.equalsIgnoreCase(n));
                ids.add(Identity.create(dr.Id, dr.Username, dr.Alias));
            });
            for (String n : names) {
                if (n.startsWith("~")) {
                    ids.add(Identity.create(n, "自动流程"));
                } else {

                    ids.add(Identity.create(n, n));
                }
            }

        }
        return ids;// new List<Security.Identity>();
    }
