...
public abstract class WebFactory implements IWebFactory {
    ...
    class XHRFlow extends WebFlow {
        ...
        public WebActivity firstActivity() {
            ...
            String stringBuilder = Utility.httpString(sb.toString());
            String eventPfx = "{\"ClientEvent\":";
            if (stringBuilder.startsWith(eventPfx)) {

                int index = stringBuilder.indexOf(",");
                if (index > -1) {
                    if ((Utility.parse(stringBuilder.substring(eventPfx.length(), index), 0) & WebEvent.ASYNCDIALOG) == WebEvent.ASYNCDIALOG) {
